import {AppError,MAX_FILE_BYTES} from './engine.mjs';
/** Same-origin file transport; computation remains in shared core. */
export async function downloadResponse(request){
 if(Number(request.headers.get('content-length')||0)>128*1024*1024)return new Response('导出内容过大',{status:413});
 ...
 if(content.length>96*1024*1024)return new Response('导出内容过大',{status:413});
 ...
 if(body.length>MAX_FILE_BYTES)return new Response('导出内容超过 32 MB，已有成果保留',{status:413});
