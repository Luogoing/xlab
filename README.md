...
当前电脑直接双击桌面“专利研习”。它会在后台启动本机服务并打开浏览器；重复打开复用同一服务，无需保持终端。地址为 http://127.0.0.1:3000 。

解压便携包后可双击 `启动专利研习.cmd`。当前电脑已验证 Node 24.13.0；其他电脑需要 Node 22.13 或更高版本。运行便携包无需安装 npm 依赖。关闭浏览器不会停止后台服务；需停止时双击包内 `停止专利研习.cmd`，有活动任务时先在页面取消。没有安装开机自启动。
...
智谱 / glm-4.7-flash 可显式选择；2026-09-14 五轮验证中，当前凭据仍返回 429/1305，未取得成功生成，所以不显示“已验证可用”。失败不自动换模型。Codex 在本机已完成气态储氢、工业余热、桥梁阻尼器的真实检索分析；前一版本还验证过液氢和电池。

## 使用流程

1. 输入主题，默认“真实专利数据库”与 Codex，直接点“搜索真实专利”。默认自动提炼主题词，再查询数据库和分析证据。首次试用可输入“工业余热回收换热器”或“桥梁振动控制阻尼器”。需要精确控制时，展开词项与日期，或关闭自动提炼并编辑实际检索式。
...
3. 点击“搜索真实专利”，观察阶段、已用时、请求数、详情数和错误。原文取得后可先阅读，不用等模型完成；窄屏可点“查看进度 / 取消”跳转。每次只允许一个活动任务。请求间隔至少 3.2 秒；还执行本机记录的 20 次/分钟、1200 次/5 小时、9000 次/周限制，单任务最多 20 次数据请求。单请求超时 35 秒，临时服务故障最多重试一次，鉴权或限流错误立即停。供应商对同一账户其他客户端的调用仍可能另行限流。
4. 阅读证据卡，可按词语或证据编号查找；报告引用会直接定位片段，可下载本机原始 XML。技术方案页有逐篇对照，窄屏可左右滑动表格。展开完整权利要求/说明书字段，记录筛选理由。取消保留会使原 AI 分析过期；点“按当前保留记录重新分析”才会再次调用。模型引用校验检查归属、逐字引文和校验和；直接引文之外的技术路线、差异及研究发现都保留“待人工复核”。程序校验不能证明归纳内容已经获得专业认可。
...
切换到新任务会保留上一任务的报告、笔记、筛选和学习反馈，可从页面顶部“已存任务”打开。不同主题不会沿用上一主题的报告标题或研究判断。
...
- `.local/workspaces`：切换时保存的旧任务，含各自的报告与人工编辑。
- `.local/gateway-quota.json`：本机调用窗口；重启后继续计数。
- `core`：离线共享核心与模型引用验证；`server`：Windows 凭据、网关、模型、任务与 HTTP。
- `docs/FIVE_ROUNDS.md`：0.2.1 五轮操作记录；`docs/ACCEPTANCE.md`：验收范围；`docs/baseline-0.1` 与交接原文：历史信息，不代表 0.2 当前能力。
